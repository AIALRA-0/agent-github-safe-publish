...
不同 Agent 容易使用不同脱敏标准，常见遗漏不只包括密码和令牌，还包括地址、个人网站、账号、UID、联系人、数据库、Git 历史、图片像素、文档属性和发行附件

本项目把安全工作拆成两个互补流程：

- 精确发布检查：只检查本次准备发布的隔离副本、Git 历史和附件；严格审计保留全部问题，发布结论只让关键风险阻止写入
- 定期暴露面审计：检查可访问 Codex 会话、已保存项目根目录和仓库关联 GitHub 表面；结果用于发现现存风险，不授权删除或修改
...
| `allow_with_risk` | 只有已经确认的非关键问题或辅助表面缺口 | 返回成功，同时保留完整风险报告 |
...
运行环境需要 Python、Git，以及全量仓库审计使用的已登录 GitHub CLI 命令行界面（Command Line Interface）
...
```powershell
python -X utf8 scripts/safe_publish.py audit-fleet --owner ExampleOrg --local-root "<LOCAL_ROOT>" --policy "$env:CODEX_HOME/private/github-safe-publish/policy.private.json" --surface-profile repository-associated --resume --output "$env:CODEX_HOME/private/github-safe-publish/fleet.private.json" --candidates-output "$env:CODEX_HOME/private/github-safe-publish/fleet-candidates.private.json" --public-summary .\fleet-summary.public.json # 分页、权限或下载失败会保留为 incomplete
```
...
- 文本：执行凭据、身份、基础设施、Unicode 归一化和有界 Base64、十六进制、URL 编码还原
- Git：检查全部可见对象、分支与标签名、注释标签、notes、作者、提交者、消息和签名载荷
- Office：检查属性、关系、嵌入文件、内嵌图片和宏
- 图片：检查元数据、全部动画帧、OCR 文字、二维码和条形码；缺少解析层时返回 `incomplete`
- PDF：检查文本、属性、附件、页面图像 OCR 和加密状态；无法检查页面内容时返回 `incomplete`
- 音视频：检查容器与流属性、可转换字幕、附件和内嵌封面；解析器或提取失败时返回 `incomplete`
...
`repository-associated` 模式覆盖：

- 协作：Issue、Pull Request、评论、Review、Discussion、标签和里程碑
- 发布：Release 元数据与附件、Wiki、GitHub Pages 元数据
- 自动化：保留的 Actions 日志、制品、变量、环境、部署、缓存元数据和权限
- 软件包：仓库关联包和容器在当前身份可读取时检查；权限不足时记录 `permission_denied`
- 安全设置：秘密扫描、推送保护、ruleset、分支保护和 Actions 权限
...
- `identifiers`：精确文字或正则规则，并声明归一化方式和适用范围
- `replacements`：人工批准的稳定合成映射
- `approved_locations`：允许出现某条规则的精确对象位置
- `blocked_paths`：禁止发布的路径模式
- `binary_approvals`：对象摘要、检查层、工具版本、批准者和复审触发器
- `exceptions`：规则、精确对象、批准者、理由、到期时间和复审触发器
- `risk_acceptances`：仓库、非关键规则、精确对象、对象摘要、扫描器摘要、批准者、理由、到期时间和复审触发器
- `schema_version`：机器格式版本

第 1 版和第 2 版策略继续读取，并只在内存中迁移；仓库文件不能扩大私有允许范围

风险接受记录只适用于固定非关键规则；对象内容、扫描器版本或到期状态变化后自动失效，凭据、私人标识、法律记录、真实数据和关键检查缺口始终不能通过该字段放行
...
Gitleaks 固定为 `v8.30.1`；该版本存在平台相关静默失效报告 [6]，因此每次进程首次使用该二进制时先运行合成凭据金丝雀；扫描器没有检出金丝雀、金丝雀超过 60 秒或仓库进程超过 330 秒时均返回 `incomplete`
...
安全问题不要粘贴候选原文、凭据或私有策略；先提交不含敏感内容的请求，让维护者提供私密渠道

仓库当前没有许可证文件；公开可见不等于授予复制、修改、再发布或商业使用权，使用前需要取得仓库所有者许可
